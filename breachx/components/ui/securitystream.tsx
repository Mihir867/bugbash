...
const SecurityLogStream: React.FC<BuildLogStreamProps> = ({ 
  deploymentUrl, 
  maxHeight = '500px',
  autoScroll = true,
  ...
  const initializeSocket = useCallback(async () => {
    ...
    if (!deploymentUrl) {
      setLogs([{ type: 'info', message: 'No deploymentUrl available. Trigger a build to see logs.' }]);
      return;
    }
    ...
    socket.on('connect', () => {
      ...
      socket.emit('subscribe-to-build', deploymentUrl);
    });
  ...
  }, [deploymentUrl, reconnectAttempt, autoScroll, scrollToBottom]);
  ...
  }, [deploymentUrl, initializeSocket]);
